...
func TestGroupAndListConversations(t *testing.T) {
	...
	if _, err := s.AppendMessage(g.ID, a.ID, "", "hello team", nil); err != nil {
		t.Fatal(err)
	}
...
func TestDenseSequencesUnderConcurrency(t *testing.T) {
	...
	for w := 0; w < senders; w++ {
		...
		go func(w int) {
			defer wg.Done()
			for i := 0; i < per; i++ {
				m, err := s.AppendMessage(conv.ID, a.ID, "", fmt.Sprintf("w%d-%d", w, i), nil)
				if err != nil {
					panic(err)
				}
				seqs <- m.Seq
...
func TestSeqRecoveryAfterCrashBeforeHint(t *testing.T) {
	...
	conv, _ := s.CreateConversation(model.ConvDM, "", a.ID, []string{b.ID})
	for i := 0; i < 5; i++ {
		if _, err := s.AppendMessage(conv.ID, a.ID, "", fmt.Sprintf("m%d", i), nil); err != nil {
			t.Fatal(err)
		}
	}
	...
	s2 := open(t, dir)
	defer s2.Close()
	m, err := s2.AppendMessage(conv.ID, a.ID, "", "after recovery", nil)
	if err != nil {
		t.Fatal(err)
	}
	if m.Seq != 8 {
...
func TestPaginationBoundaries(t *testing.T) {
	...
	const total = 105
	for i := 1; i <= total; i++ {
		if _, err := s.AppendMessage(conv.ID, a.ID, "", fmt.Sprintf("msg-%d", i), nil); err != nil {
			t.Fatal(err)
		}
	}
...
func TestReopenPreservesEverything(t *testing.T) {
	...
	conv, _ := s.CreateConversation(model.ConvDM, "", a.ID, []string{b.ID})
	for i := 0; i < 20; i++ {
		s.AppendMessage(conv.ID, a.ID, "", fmt.Sprintf("m%d", i), nil)
	}
	s.Close()
